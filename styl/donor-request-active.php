...
if($_GET['action']=="delete_typing" && $_GET['h']==sha1($_GET['ID']."SaltIsGoodForLife45")):
	$SQL=mysql_query("DELETE FROM typing_request_donor WHERE ID_typing='".mysql_real_escape_string($_GET['ID'])."' AND DonorNumber='".mysql_real_escape_string($_GET['DonorNum'])."'");
...
if($_GET['action']=="delete_sample" && $_GET['h']==sha1($_GET['ID']."SaltIsGoodForLife45")):
	$SQL=mysql_query("DELETE FROM sample_request_donor WHERE ID_sample='".mysql_real_escape_string($_GET['ID'])."' AND DonorID='".mysql_real_escape_string($_GET['DonorID'])."' LIMIT 1");
	if(mysql_affected_rows()>0):
		message(1, "Item deleted", "", "");
	else:
		message(1, "Delete failed", "", "");
...
if($_SESSION['usr_ID_role']!=1):
	$where.=" AND typing_request.RegID='".mysql_real_escape_string($_SESSION['usr_RegID'])."'";
	$where2.=" AND sample_request.RegID='".mysql_real_escape_string($_SESSION['usr_RegID'])."'";
	$where3.=" AND workup_request.RegID='".mysql_real_escape_string($_SESSION['usr_RegID'])."'";
	
	if($_SESSION['usr_InstID']):
		$where.=" AND typing_request.InstID='".mysql_real_escape_string($_SESSION['usr_InstID'])."'";
		$where2.=" AND sample_request.InstID='".mysql_real_escape_string($_SESSION['usr_InstID'])."'";
		$where3.=" AND workup_request.InstID='".mysql_real_escape_string($_SESSION['usr_InstID'])."'";
	endif;
endif;
...
$vysledek = mysql_query("SELECT 1 AS druh, typing_request.ID, typing_request.date_completing AS datum_request, typing_request.patient_name, typing_request.PatientNum, typing_request.diagnosis, typing_request.RegID, typing_request.InstID, 0 AS smp_arr_date,
	...
	echo("SELECT 1 AS druh, typing_request.ID, typing_request.date_completing AS datum_request, typing_request.patient_name, typing_request.PatientNum, typing_request.diagnosis, typing_request.RegID, typing_request.InstID, 0 AS smp_arr_date,
         	typing_request_donor.DonorID, typing_request_donor.ID_stavu, typing_request_donor.duvod, typing_request_donor.DonorNumber, typing_request_donor.aLogMsgNum
         	FROM typing_request_donor LEFT JOIN typing_request ON(typing_request_donor.ID_typing=typing_request.ID)
         	WHERE ID_stavu IN(0,2,1,3) $where
         	GROUP BY DonorNumber, ID_typing
		...
		echo "<table cellspacing=\"0\" width=\"100%\">
			...
			while($zaz = mysql_fetch_array($vysledek)){
				...
				if($druh==2 || $druh==3):	//Sample, Workup
					...
					WHERE PatientNum='".mysql_real_escape_string($PatientNum)."' AND RegID='".mysql_real_escape_string($RegID)."' AND InstID='".mysql_real_escape_string($InstID)."' ORDER BY datum_vlozeni DESC LIMIT 1");
						if(mysql_num_rows($vysledek_sub) > 0):
							$zaz_sub = mysql_fetch_array($vysledek_sub);
								extract($zaz_sub);
						endif;
				...
				WHERE request_response.RegID='".mysql_real_escape_string($RegID)."' AND request_response.RequestID='".mysql_real_escape_string($ID)."' 
				AND request_response.RequestType='".mysql_real_escape_string($druh)."'
				"), 0);
			
				echo "<tr ".$styl.">";
					echo "<td>".date($_SESSION['date_format_php'],htmlspecialchars($datum_request))."</td>";
					echo "<td>".htmlspecialchars($RegID).htmlspecialchars($PatientNum)."P</td>";
					...
					if($ID_stavu==0 || $ID_stavu==2):
						if($druh==1):	//Typing
							echo "<a href=\"$_SERVER[PHP_SELF]?action=delete_typing&amp;ID=$ID&amp;DonorNum=$DonorNumber&amp;h=".$hash."\" title=\"DELETE\" id=\"doDelete1\">".$ico_smazat."</a>";
						endif;
						if($druh==2):	//sample
							echo "<a href=\"$_SERVER[PHP_SELF]?action=delete_sample&amp;ID=$ID&amp;DonorID=$DonorID&amp;h=".$hash."\" title=\"DELETE\" id=\"doDelete2\">".$ico_smazat."</a>";
						endif;
						if($druh==3):	//workup
							echo "<a href=\"$_SERVER[PHP_SELF]?action=delete_workup&amp;ID=$ID&amp;h=".$hash."\" title=\"DELETE\" id=\"doDelete3\">".$ico_smazat."</a>";
						endif;
