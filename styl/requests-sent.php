...
if($_GET['action']=="delete" && $_GET['h']==sha1($_GET['ID']."SaltIsGoodForLife45") ):
	$SQL=mysql_query("DELETE FROM search_request WHERE ID='".mysql_real_escape_string($_GET['ID'])."' AND PatientNum IS NULL LIMIT 1");
	...
	foreach($_POST as $klic=>$hodnota):
		...
		if($broken[0]=="cb" && $hodnota>0):
			...
			if($_POST['operation']==1):	//suspended
				...
				$vysledek2 = mysql_query("UPDATE search_request SET ID_stavu='7', datum_odeslani='0', datum_zpracovani='0', duvod_zmeny_stavu='".mysql_real_escape_string($_POST['duvod'])."' WHERE ID='".mysql_real_escape_string($hodnota)."' AND PatientNum IS NOT NULL LIMIT 1"); //ty s PatientNum musi zadat o presun
			...
			if($_POST['operation']==2):	//stopped
				...
				$vysledek2 = mysql_query("UPDATE search_request SET ID_stavu='6', datum_odeslani='0', datum_zpracovani='0', duvod_zmeny_stavu='".mysql_real_escape_string($_POST[$duvod_name])."' WHERE ID='".mysql_real_escape_string($hodnota)."' AND PatientNum IS NOT NULL LIMIT 1"); //ty s PatientNum musi zadat o presun
			...
			if($_POST['operation']==3):	//to active
				$vysledek = mysql_query("UPDATE search_request SET ID_stavu='8', datum_zpracovani='0', datum_odeslani='0', duvod_zamitnuti='', duvod_zmeny_stavu='' WHERE ID='".mysql_real_escape_string($hodnota)."' AND PatientNum IS NOT NULL LIMIT 1"); //zadost o presun do active
			    echo("UPDATE search_request SET ID_stavu='8', datum_zpracovani='0', datum_odeslani='0', duvod_zamitnuti='', duvod_zmeny_stavu='' WHERE ID='".mysql_real_escape_string($hodnota)."' AND PatientNum IS NOT NULL LIMIT 1");
				if($vysledek):
					$zpracovano++;
				else:
					$nezpracovano++;
...
//-----vypis dat
$vysledek = mysqli_prepare($mysqli,"SELECT search_request.ID, RegID, ID_stavu, date_request, last_name, first_name, InstID, duvod_zamitnuti, search_request.PatientNum, search_request_stavy.stav, search_request_stavy.upresneni
FROM search_request LEFT JOIN search_request_stavy ON(search_request.ID_stavu=search_request_stavy.ID) WHERE $where $where_hledej ORDER BY ? LIMIT ?, ?");
echo("SELECT search_request.ID, RegID, ID_stavu, date_request, last_name, first_name, InstID, duvod_zamitnuti, search_request.PatientNum, search_request_stavy.stav, search_request_stavy.upresneni
     FROM search_request LEFT JOIN search_request_stavy ON(search_request.ID_stavu=search_request_stavy.ID) WHERE $where $where_hledej ORDER BY ? LIMIT ?, ?");
    mysqli_stmt_bind_param($vysledek, "sii", $orderdata,$_od,$_na_stranku);
	//echo("=".$orderdata."=<br>");
	mysqli_stmt_execute($vysledek) or die("Error: ".mysqli_error($mysqli));
    mysqli_stmt_store_result($vysledek);
    $result = mysqli_stmt_bind_result($vysledek);
	if ($DEBUG_CFG==1)echo mysql_error();
	if ($DEBUG_CFG==1)echo mysqli_error($mysqli);
	$num = mysqli_stmt_num_rows($vysledek);
	...
	if($num > 0):
		...
		echo "<table cellspacing=\"0\" width=\"100%\">
			...
			while($zaz = mysqli_fetch_array($result)){
