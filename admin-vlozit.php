...
if($_POST['action']=="upravit"):
	...
	if($SQL):	//--pokud update neco zmenil
		...
		//--OPRAVNENI k modulum
		mysql_query("DELETE FROM admin_prava WHERE ID_admin='".clean_high($_GET['ID'])."' LIMIT 1");
...
if(!$_POST['action']):
	...
	$vysledek = mysql_query("SELECT * FROM admin WHERE ID='".clean_high($_GET['ID'])."' LIMIT 1");
	if(mysql_num_rows($vysledek)>0):
		$zaz = mysql_fetch_array($vysledek);
			...
			$vysledek_p = mysql_query("SELECT modul FROM admin_prava WHERE ID_admin='".clean_high($_GET['ID'])."'");
	...
	echo "<form onsubmit=\"return Kontrola"; if($_GET['ID']): echo "2"; endif; echo "();\" enctype=\"multipart/form-data\" method=\"POST\" action=\"\" name=\"formular\">
		...
		echo "<div class=\"form-lr\">
			...
			<div class=\"form-right\"><select name=\"ID_registru\" style=\"width:".$width6."px;\">";

					echo "<OPTION class=\"form\" value=\"\">-choose the Register-</option>";

					$vysledek_if = mysql_query("SELECT registers.ID AS IDregistru, registers.registr, transplant_centers.centrum FROM registers LEFT JOIN transplant_centers ON(registers.ID_centra=transplant_centers.ID) ORDER BY transplant_centers.centrum, registers.registr");
					if(mysql_num_rows($vysledek_if)):
						while($zaz_if = mysql_fetch_array($vysledek_if)):
							extract($zaz_if);
							
							echo "<OPTION class=\"form\" value=\"$IDregistru\""; if($ID_registru==$IDregistru): echo "selected"; endif; echo ">$registr ($centrum)</option>";
		...
		*/
		...
		<div class=\"form-lr\" style=\"margin-top:20px;\">
			...
			<div class=\"form-right\"><select name=\"width\">";
					...
					//echo "<OPTION class=\"form\" value=\"\">dle rozli�en� obrazovky</option>";
		...
		echo "<div class=\"form-lr\" style=\"margin-top:20px;\">
			<div class=\"form-left\">Po�adavky �adit dle:</div>
			<div class=\"form-right\"><select name=\"razeni\">";

					echo "<OPTION class=\"form\" value=\"ID\""; if($razeni=="ID"): echo " selected"; endif; echo ">data vlo�en� (v�choz� nastaven�)</option>";
		...
		*/
		...
		/*
		echo "<div class=\"form-lr\">
			<div class=\"form-left\">V�choz� jazyk:</div>
			<div class=\"form-right\"><select name=\"jazyk_vychozi\">";

					echo "<OPTION class=\"form\" value=\"_cz\""; if($jazyk_vychozi=="_cz"): echo " selected"; endif; echo ">�esk�</option>";
					echo "<OPTION class=\"form\" value=\"_en\""; if($jazyk_vychozi=="_en"): echo " selected"; endif; echo ">anglick�</option>";
		...
		*/
		...
		echo "<div class=\"form-lr\" style=\"margin-top:35px;\" id=\"opravneni_moduly\">
			...
			<div class=\"form-right\">";
				...
				echo "<div style=\"float:left; margin-bottom:12px;\"><select name=\"ID_role_admin\" id=\"ID_role_admin\" onchange=\"nastav_moduly();\">";
...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function Kontrola ()
  {
		...
		if(!document.formular.platnost_od.value || document.formular.platnost_od.value=="dd.mm.rrrr")
  		{
