		...
		if($broken[0]=="cb" && $hodnota>0):
			...
			*/
			...
			if($_POST['operation']==8):	//stopped
				...
				$vysledek2 = mysql_query("UPDATE search_request SET ID_stavu='6', datum_odeslani='0', datum_zpracovani='0', duvod_zmeny_stavu='".mysql_real_escape_string($_POST[$duvod_name])."', datum_zmeny_stavu=NOW() WHERE ID='".mysql_real_escape_string($hodnota)."' AND PatientNum IS NOT NULL LIMIT 1"); //ty s PatientNum musi zadat o presun
			...
			if($_POST['operation']==2):	//suspended
				...
				$vysledek2 = mysql_query("UPDATE search_request SET ID_stavu='7', datum_odeslani='0', datum_zpracovani='0', duvod_zmeny_stavu='".mysql_real_escape_string($_POST['duvod'])."', datum_zmeny_stavu=NOW() WHERE ID='".mysql_real_escape_string($hodnota)."' AND PatientNum IS NOT NULL LIMIT 1"); //ty s PatientNum musi zadat o presun
...
//	Stránkování
//============================================================================= */
strankovani("search_request LEFT JOIN search_request_stavy ON(search_request.ID_stavu=search_request_stavy.ID) WHERE search_request.ID_stavu IN(4,5) $where_hledej $where", $_na_stranku);
...
//-----vypis dat
$vysledek = mysql_query("SELECT search_request.ID, RegID, ID_stavu, date_request, last_name, first_name, InstID, duvod_zamitnuti, PatientNum, search_request_stavy.stav, search_request_stavy.upresneni  
FROM search_request LEFT JOIN search_request_stavy ON(search_request.ID_stavu=search_request_stavy.ID) 
WHERE search_request.ID_stavu IN(4,5) $where_hledej $where ORDER BY $orderby LIMIT $_od, $_na_stranku");
	...
	if($num > 0):
		...
		echo "<table cellspacing=\"0\" width=\"100%\">
			...
			echo "<select name=\"duvod2\" id=\"duvod2\" style=\"width:400px;\">";
				...
				$vysledek_sub = mysql_query("SELECT ID AS ID_duvod, duvod FROM nastaveni_duvody_prepnuti WHERE ID>=20 AND ID<=24 ORDER BY ID");
					if(mysql_num_rows($vysledek_sub)>0):
						while($zaz_sub = mysql_fetch_assoc($vysledek_sub)):
							extract($zaz_sub);
