...
//class WebApp{
	...
	function ConfirmPreliminaryReq($ID,$PatientNum){
		if(autentizace()){
			
			$vysledek = mysql_query("UPDATE search_request SET ID_stavu='2', datum_zpracovani='".time()."', duvod_zamitnuti='', PatientNum='".mysql_real_escape_string($PatientNum)."', datum_zmeny_stavu=NOW() WHERE ID='".mysql_real_escape_string($ID)."' LIMIT 1");
			if(mysql_affected_rows()>0):
				$return_code=1;
			else:
				$return_code=5004;
		...
		if(autentizace()){
			...
			if($xml):
				foreach ($xml->Donor as $Donor):
					...
					$vysledek_check = mysql_result(mysql_query("SELECT COUNT(ID) FROM patient_donor WHERE DonorNumber='$DonorNumber' AND PatientNum='$PatientNum'"), 0);
					if($vysledek_check):
						$pole_update=array();
						$citac=0;
						...
						$dotaz="UPDATE patient_donor SET ".implode(", ",$pole_update).", datum_vlozeni='$dnes' WHERE DonorNumber='$DonorNumber' AND PatientNum='$PatientNum' LIMIT 1";
						//echo $dotaz."<br><br>";
						...
						$dotaz="INSERT INTO patient_donor (ID, RegID, PatientNum, ".implode(", ",$pole_sloupcu).", datum_vlozeni) VALUES ('NULL', '$RegID', '$PatientNum', '".implode("', '",$pole_hodnot)."', '$dnes')";
						$vysledek = mysql_query($dotaz);
						if($vysledek):
							$zaznam.="<return_code>1</return_code>";
						else:
	...
	function SetActivePatients($RegID,$PatientNum){
		if(autentizace()){
		
			$vysledek = mysql_query("UPDATE search_request SET ID_stavu='2', datum_zpracovani='".time()."', datum_zmeny_stavu=NOW() WHERE RegID='".mysql_real_escape_string($RegID)."' AND PatientNum='".mysql_real_escape_string($PatientNum)."' LIMIT 1");
	...
	* @param  string
	...
	function ConfirmTypingReq($RegID, $ID, $DonorNum, $aLogMsgNum){
		if(autentizace()){
			...
			WHERE typing_request.RegID='$RegID' AND typing_request_donor.ID_typing='".mysql_real_escape_string($ID)."' AND typing_request_donor.DonorNumber='".mysql_real_escape_string($DonorNum)."'");
		...
		if(autentizace()){
			...
			if($xml):
				...
				foreach ($xml->form as $Typing):
					...
					unset($DonorNumber,$DonorID,$resolution,$aLogMsgNum);
		...
		if(autentizace()){
			...
			if($xml):
				...
				foreach ($xml->form as $Sample):
					...
					unset($DonorNumber,$DonorID,$aLogMsgNum);
...
//SMAZAT pri vytvareni WSDL
	...
	//$server = new SoapServer("http://www.tvorbawww.cz/registr/soap/definice.wsdl?wsdl");
