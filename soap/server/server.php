...
//SMAZAT pri vytvareni WSDL {
	require('1-function-mail.php');
	...
	function send_email($InstID, $msg_id, $RegID="", $PatientNum=""){
		
		$_chyby=array();
		
		if(!isset($RegID)):
			$_chyby[]="není isset RegID (".$RegID.")";
		endif;
		if(empty($RegID)):
			//Je to divne, ale $RegID je hodnoceno jako empty, pritom echo $RegID vypise hodnotu.
			$_chyby[]="je empty RegID (".var_dump($RegID)."/".$RegID.")";
		endif;
		
		
		
		//email centra kam zaslat
		$email="";
		$vysledek_msg = mysql_query("SELECT email FROM transplant_centers WHERE InstID='".mysql_real_escape_string($InstID)."' LIMIT 1");
		$error=mysql_error();
		if($error):
			$_chyby[]=$error;
		endif;
			if(mysql_num_rows($vysledek_msg)>0):
				$zaz_msg = mysql_fetch_assoc($vysledek_msg);
					extract($zaz_msg);
					
					if(!$email):
						$_chyby[]="není email (".$email.")";
					endif;
			endif;
		@mysql_free_result($vysledek_msg);
		
		//dohledat Hospital patient ID
		$hop_id="";
		//if(empty($RegID) || empty($PatientNum)):
			//$_chyby[]="není RegID (".$RegID.") nebo PatientNum (".$PatientNum.")";
		//else:
			$hop_id="";
			$vysledek_msg = mysql_query("SELECT patient_id AS hop_id FROM search_request WHERE RegID='".mysql_real_escape_string($RegID)."' AND PatientNum='".mysql_real_escape_string($PatientNum)."' LIMIT 1");
			$error=mysql_error();
			if($error):
				$_chyby[]=$error;
			endif;			
				if(mysql_num_rows($vysledek_msg)>0):
					$zaz_msg = mysql_fetch_assoc($vysledek_msg);
						...
						if(!$hop_id):
							$_chyby[]="není patient_id (".$hop_id."), RegID: ".$RegID.", PatientNum: ".$PatientNum;
						endif;
				else:
					$_chyby[]="nenalezeno patient_id";
				endif;
			@mysql_free_result($vysledek_msg);
		//endif;
		
		
		$ident=$RegID.$PatientNum."P";
		
		if($email):
			$vysledek_msg = mysql_query("SELECT subject AS typ_updatu, msg, url FROM setting_email_msg WHERE ID='".mysql_real_escape_string($msg_id)."' LIMIT 1");
			$error=mysql_error();
			if($error):
				$_chyby[]=$error;
			endif;
				if(mysql_num_rows($vysledek_msg)>0):
					$zaz_msg = mysql_fetch_assoc($vysledek_msg);
						...
						//$msg=str_replace("%IDENT%",$ident,$msg);
						
						$subject="TC web notification - Patient ".$hop_id." / ".$ident."";
						$msg="You received an update from the registry (".$typ_updatu.") for you patient ".$hop_id." / ".$ident.". You can check it <a href=\"".WEB_URL.$url."\" target=\"_blank\">here</a>";
						
						if($msg):
							$navrat=zasli_email('', $email, $subject, $msg);
							
							if(!$navrat):
								$_chyby[]="neodeslal se email";
							endif;
		...
		if(!empty($_chyby)):
			foreach($_chyby as $chyba):
				mysql_query("INSERT INTO log_notification (msg, msg_id, date) VALUES 
					('".mysql_real_escape_string($chyba)."',
...
// } konec smazani pri vytvareni WSDL



//Odkomentovat tridu pri vytvareni WSDL
//class WebApp{
	...
	function ConfirmPreliminaryReq($ID,$PatientNum,$status=""){
		if(autentizace()){
			...
			*/
			...
			if($pnum_je):
				...
				if(mysql_affected_rows()>0):
					$return_code=1;
					send_email($InstID,1,$RegID,$PatientNum);
				else:
					if(mysql_errno() == 1062):	//duplicita PatientNum
						$return_code=5006;
					else:
						$return_code=5004;
					endif;
	...
	function DeniedPreliminaryRequestBy($ID,$duvod){
		if(autentizace()){
			...
			//dohledani udaju
			$vysledek = mysql_query("SELECT RegID, InstID, PatientNum FROM search_request WHERE ID='".mysql_real_escape_string($ID)."' LIMIT 1");
				if(mysql_num_rows($vysledek)>0):
					$zaz = mysql_fetch_assoc($vysledek);
						extract($zaz);
				endif;
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,2,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	function SetPreliminaryReq($ID){
		if(autentizace()){
			...
			//dohledani udaju
			$vysledek = mysql_query("SELECT InstID, RegID, PatientNum FROM search_request WHERE ID='".mysql_real_escape_string($ID)."' LIMIT 1");
				if(mysql_num_rows($vysledek)>0):
					$zaz = mysql_fetch_assoc($vysledek);
						extract($zaz);
				endif;
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,3,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
				...
				foreach ($xml->Donor as $Donor):
						...
						if($atribut=="RecordUpdate"):
							if($data!=""):
								if(strpos($data,"-")):
								   ...
								   $data=MkTime(0,0,0,$explode[1],$explode[2],$explode[0]);
								elseif((strpos($data,"."))):
									$explode=explode(".",$data);
									$data=MkTime(0,0,0,$explode[1],$explode[2],$explode[0]);
								endif;
					...
					if($vysledek_check):
						...
						if(mysql_affected_rows()>0):
							...
							send_email($InstID,4,$RegID,$PatientNum);
						else:
						...
						if($vysledek):
							$zaznam.="<return_code>1</return_code>";
							send_email($InstID,5,$RegID,$PatientNum);
						else:
							$zaznam.="<return_code>5004</return_code>";
						endif;
					endif;
	...
	function GetStoppedPatients($RegID){
		if(autentizace()){
			...
			<request>";
				...
				$vysledek = mysql_query("SELECT ID, PatientNum, RegID, temp_transpl_date, duvod_zmeny_stavu, DATE_FORMAT(datum_zmeny_stavu,'%d.%m.%Y %H:%i') AS datum_zmeny_stavu2 FROM search_request WHERE ID_stavu='6' AND PatientNum>0 AND RegID='".mysql_real_escape_string($RegID)."' ORDER BY ID");
					if(mysql_num_rows($vysledek)>0):
						while($zaz = mysql_fetch_assoc($vysledek)):
							extract($zaz);
							
							if($temp_transpl_date<>0): $temp_transpl_date=date("d.m.Y",$temp_transpl_date); else: $temp_transpl_date=""; endif;
	...
	function GetSuspendedPatients($RegID){
		if(autentizace()){
			...
			<request>";
				...
				$vysledek = mysql_query("SELECT ID, PatientNum, RegID, temp_transpl_date, duvod_zmeny_stavu, DATE_FORMAT(datum_zmeny_stavu,'%d.%m.%Y %H:%i') AS datum_zmeny_stavu2 FROM search_request WHERE ID_stavu='7' AND PatientNum>0 AND RegID='".mysql_real_escape_string($RegID)."' ORDER BY ID");
					if(mysql_num_rows($vysledek)>0):
	...
	function GetActivePatients($RegID){
		if(autentizace()){
			...
			<request>";
				...
				$vysledek = mysql_query("SELECT ID, PatientNum, temp_transpl_date, RegID, DATE_FORMAT(datum_zmeny_stavu,'%d.%m.%Y %H:%i') AS datum_zmeny_stavu2 FROM search_request WHERE ID_stavu='8' AND PatientNum>0 AND RegID='".mysql_real_escape_string($RegID)."' ORDER BY ID");
					if(mysql_num_rows($vysledek)>0):
						while($zaz = mysql_fetch_array($vysledek)):
							extract($zaz);
	...
	function SetStoppedPatients($RegID,$PatientNum){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,6,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	function SetSuspendedPatients($RegID,$PatientNum){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,7,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	function SetActivePatients($RegID,$PatientNum,$status=0){
		if(autentizace()){
		
			mysql_query("INSERT INTO status_test (ID, RegID, PatientNum, status, datum) VALUES (
			NULL, 
			'".mysql_real_escape_string($RegID)."', 
			'".mysql_real_escape_string($PatientNum)."',
			...
			*/
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,8,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
								...
								if(mysql_num_rows($vysledek8)>0):
									$zaz8 = mysql_fetch_array($vysledek8);
										...
										if($temp_transpl_date<>0):
											$temp_transpl_date=date("d.m.Y",$temp_transpl_date); else: $temp_transpl_date="";
										endif;
								endif;
	...
	* @param  string
	...
	function ConfirmTypingReq($RegID, $ID, $DonorNum, $aLogMsgNum){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,9,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	* @param  string
	...
	function DenyTypingReq($RegID, $ID, $DonorNum, $duvod){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,10,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	* @param  integer
	...
	function DenySendedTypingReq($RegID, $aLogMsgNum, $duvod){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,11,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
		...
		if(autentizace()){
			...
			if($xml_data):
				...
				foreach ($xml_data->form as $Result):
					...
					$cislo_existuje = mysql_result(mysql_query("SELECT COUNT(*) FROM typing_request_donor WHERE aLogMsgNum='".mysql_real_escape_string($aLogMsgNum)."'"), 0);
					...
					if($cislo_existuje):
						...
						$vysledek_check = mysql_result(mysql_query("SELECT COUNT(*) FROM typing_result WHERE RegID='".mysql_real_escape_string($RegID)."' AND aLogMsgNum='".mysql_real_escape_string($aLogMsgNum)."'"), 0);
						if($vysledek_check):
							$pole_update=array();
							$citac=0;
							foreach($pole_sloupcu as $sloupec):
							...
							$dotaz="UPDATE typing_result SET ".implode(", ",$pole_update).", datum_vlozeni='$dnes' WHERE RegID='".mysql_real_escape_string($RegID)."' AND aLogMsgNum='".mysql_real_escape_string($aLogMsgNum)."' LIMIT 1";
							//echo $dotaz."<br><br>";
							$vysledek = mysql_query($dotaz);
							if($vysledek):
								if(mysql_affected_rows()>0):
									...
									send_email($InstID,12,$RegID,$PatientNum);
								else:
									unset($dotaz);
								endif;
							endif;
						else:
							$dotaz="INSERT INTO typing_result (ID, RegID, aLogMsgNum, ".implode(", ",$pole_sloupcu).", datum_vlozeni) VALUES ('NULL', '".mysql_real_escape_string($RegID)."', '".mysql_real_escape_string($aLogMsgNum)."', '".implode("', '",$pole_hodnot)."', '$dnes')";
							$vysledek = mysql_query($dotaz);
							if($vysledek):
								$probehlo=1;
								$importovano++;
								
								send_email($InstID,13,$RegID,$PatientNum);
							endif;
						endif;
	...
	* @param  string
	...
	function ConfirmSampleReq($RegID, $ID, $DonorNum, $aLogMsgNum){
		if(autentizace()){
			...
			WHERE sample_request.RegID='".mysql_real_escape_string($RegID)."' AND sample_request_donor.ID_sample='".mysql_real_escape_string($ID)."' AND sample_request_donor.DonorNumber='".mysql_real_escape_string($DonorNum)."' LIMIT 1");
				if(mysql_num_rows($vysledek)>0):
					$zaz = mysql_fetch_assoc($vysledek);
						extract($zaz);
				endif;
			...
			WHERE sample_request.RegID='".mysql_real_escape_string($RegID)."' AND sample_request_donor.ID_sample='".mysql_real_escape_string($ID)."' AND sample_request_donor.DonorNumber='".mysql_real_escape_string($DonorNum)."'");

			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,14,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	* @param  string
	...
	function DenySampleReq($RegID, $ID, $DonorNum, $duvod){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,15,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	* @param  integer
	...
	function DenySendedSampleReq($RegID, $aLogMsgNum, $duvod){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,16,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	function ConfirmWorkupReq($ID,$aLogMsgNum){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,17,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
	...
	function DenyWorkupRequest($ID,$duvod){
		if(autentizace()){
			...
			if(mysql_affected_rows()>0):
				$return_code=1;
				send_email($InstID,18,$RegID,$PatientNum);
			else:
				$return_code=5004;
			endif;
				...
				foreach ($xml->form as $Patient):
					...
					if($pnum_je):
						...
						if($vysledek):
							...
							send_email($InstID,19,$RegID,$PatientNum);
						else:
							if(mysql_errno() == 1062):	//duplicita PatientNum
								$xml_data.="<return_code>5006</return_code>";
							else:
				...
				foreach ($xml->form as $Typing):
					...
					if($idcko>0 && $DonorNumber):
						...
						for($i=1;$i<=3;$i++):
							...
							if($vysledek):
								$xml_data="<return_code>1</return_code>";
								send_email($InstID,20,$RegID,$PatientNum);
							else:
								$xml_data="<return_code>5004</return_code>";
							endif;
						endfor;
			...
			if($xml):
				...
				foreach ($xml->form as $Sample):
					...
					if($idcko>0 && $DonorNumber):
						...
						if($vysledek):
							$xml_data="<return_code>1</return_code>";
							send_email($InstID,21,$RegID,$PatientNum);
						else:
							$xml_data="<return_code>5004</return_code>";
						endif;
			...
			if($xml):
				...
				foreach ($xml->form as $Workup):
					...
					if($vysledek):
						...
						$xml_data.="<return_code>1</return_code>
						<ID>".$idcko."</ID>";
						send_email($InstID,22,$RegID,$PatientNum);
					else:
						$xml_data.="<return_code>5004</return_code>";
					endif;
					...
					if(!$pacient_je):
						...
						if($pnum_je):
							...
							if($vysledek):
								...
								//dohledani udaju
								$vysledek_sub = mysql_query("SELECT InstID, PatientNum FROM search_request WHERE ID='".mysql_real_escape_string($ID)."' LIMIT 1");
									if(mysql_num_rows($vysledek_sub)>0):
										$zaz_sub = mysql_fetch_assoc($vysledek_sub);
											extract($zaz_sub);
									endif;
								...
								if($PatientNum):
									send_email($InstID,23,$RegID,$PatientNum);
								else:
									send_email($InstID,23,$RegID,$PatientNum);
								endif;
							else:
								if(mysql_errno() == 1062):	//duplicita PatientNum
									...
									$xml_data.="<return_code>5004</return_code>
									<return_msg>".mysql_error()."</return_msg>";
								endif;
							endif;
						endif;
					endif;
								...
								if($DonorID):
									...
									for($i=1;$i<=3;$i++):
										...
										if($vysledek):
											if($i==1):
												$xml_data.="<return_code>1</return_code>";
												send_email($InstID,24,$RegID,$PatientNum);
											endif;
										else:
											if($i==1):
												$xml_data="<return_code>5004</return_code>";
					...
					if($citac_form==1):
						...
						if(!$zaznam_je):
							...
							if($vysledek):
								...
								send_email($InstID,25,$RegID,$PatientNum);
							else:
								$xml_data.="<return_code>5004</return_code>";
							endif;
						endif;
					...
					endforeach;
						...
						if(!$zaznam_je):
							...
							if($vysledek):
								...
								send_email($InstID,26,$RegID,$PatientNum);
							else:
								$xml_data.="<return_code>5004</return_code>";
							endif;
						endif;
...
//SMAZAT pri vytvareni WSDL
	...
	if($_SERVER['HTTP_HOST']=="127.0.0.1"):
		$server = new SoapServer("http://127.0.0.1/swe/soap/definice.wsdl?wsdl");
	else:
		//$server = new SoapServer("http://tr.dpa.nu/soap/definice.wsdl?wsdl");
		$server = new SoapServer("http://tc.hemeos.com/soap/definice.wsdl?wsdl");
	endif;


	$server->addFunction("autentizace");
	$server->addFunction("GetTest");
	$server->addFunction("GetPreliminaryRequests");
	$server->addFunction("ConfirmPreliminaryReq");
	$server->addFunction("DeniedPreliminaryRequestBy");
	$server->addFunction("GetDeniedRequests");
	$server->addFunction("SetPreliminaryReq");
	$server->addFunction("SendDonorForPatient");
	$server->addFunction("GetStoppedPatients");
	$server->addFunction("GetSuspendedPatients");
	$server->addFunction("GetActivePatients");
	$server->addFunction("SetStoppedPatients");
	$server->addFunction("SetSuspendedPatients");
	$server->addFunction("SetActivePatients");
	$server->addFunction("GetTypingRequests");
	$server->addFunction("ConfirmTypingReq");
	$server->addFunction("DenyTypingReq");
	$server->addFunction("DenySendedTypingReq");
	$server->addFunction("SendResultOfTyping");
	$server->addFunction("GetSampleRequests");
	$server->addFunction("ConfirmSampleReq");
	$server->addFunction("DenySampleReq");
	$server->addFunction("DenySendedSampleReq");
	$server->addFunction("GetWorkupRequests");
	$server->addFunction("ConfirmWorkupReq");
	$server->addFunction("DenyWorkupRequest");
	$server->addFunction("SendNewPatient");
	$server->addFunction("SendNewTypingRequest");
	$server->addFunction("SendNewSampleRequest");
	$server->addFunction("SendNewWorkupRequest");
	$server->addFunction("UpdatePatient");
	$server->addFunction("UpdateTypingRequest");
	$server->addFunction("UpdateSampleRequest");
	$server->addFunction("UpdateWorkupRequest");
	$server->addFunction("SendReqResponse");
...
//Odkomentovat pri vytvareni WSDL
/*
