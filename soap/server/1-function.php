...
function strankovani($dotaz, $_na_stranku, $pocitej_sloupec="*"){
	...
	$zaznamu=mysql_result(mysql_query("SELECT COUNT($pocitej_sloupec) FROM $dotaz"), 0);
...
function strankovani2(){
	...
	if($_stranek>1):
		...
		echo "<div style=\"float:left; width:100%; margin-top:20px;\">
			<div style=\"float:left; width:100px; text-align:left; margin-left:12px; display:inline;\">"; if($_stranek): echo "Strana: $_GET[page]/$_stranek"; endif; echo "</div>
			...
			$query_string=preg_replace("(page=[0-9]+&)", "", $query_string);
				...
				if($_GET['page']>1):
					$kam=$_GET['page']-1;
					echo "<a href=\"$_SERVER[PHP_SELF]?page=$kam&amp;$query_string\" class=\"odkaz3\"><</a> &nbsp;";
				endif;
				...
				for($i=1;$i<=$_stranek;$i++):
						if($_GET['page']==$i):
							...
							echo "<a href=\"$_SERVER[PHP_SELF]?page=$i&amp;$query_string\" class=\"odkaz3\">$i</a>";
						endif;
...
function order($text, $sloupec){
	...
	$odkaz="<a href=\"$_SERVER[PHP_SELF]?col=$sloupec&amp;$get_name=$_GET[$get_name]&amp;$query_string\" class=\"odkaz2\">$text</a>";
...
function razeni($primarne){
	...
	if($_GET['col']):
		$orderby=$_GET['col']." ".$_GET['ord_'.str_replace(".","",$_GET[col])];
	else:
		$orderby=$primarne;
	endif;
} 
...
function get_id_adresatu($ID_projektu,$prava,$prava_rozsirujici){
	...
	$vysledek_p = mysql_query("SELECT ID_uzivatele AS id_adresata FROM admin_prava_projekty WHERE ID_projektu='$ID_projektu' AND prava='$prava' AND prava_rozsirujici='$prava_rozsirujici'");
		if(mysql_num_rows($vysledek_p)):
			while($zaz_p = mysql_fetch_array($vysledek_p)):
				extract($zaz_p);
...
function get_id_adresatu_zadavatelu($ID_pozadavku){
	...
	$vysledek_p = mysql_query("SELECT ID_zadavatele AS id_adresata FROM _pozadavky_zadavatele WHERE ID_pozadavku='$ID_pozadavku'");
		if(mysql_num_rows($vysledek_p)):
			while($zaz_p = mysql_fetch_array($vysledek_p)):
				extract($zaz_p);
				$_pole_id_adresatu[]=$id_adresata;
...
function zasli_email_adresatum($_pole_id_adresatu,$druh_zpravy,$autor_udalosti_id,$ID_pozadavku){
	...
	if(count($_pole_id_adresatu)>0):
		...
		$vysledek_p = mysql_query("SELECT CONCAT(jmeno,' ',prijmeni) AS autor, admin.email AS email_od FROM admin WHERE admin.ID='$autor_udalosti_id'");
			if(mysql_num_rows($vysledek_p)):
		...
		$vysledek_p = mysql_query("SELECT ID_projektu, nazev AS nazev_poz, pozadavek AS poz, reakce AS reakce_poz FROM pozadavky WHERE ID='$ID_pozadavku'");
			if(mysql_num_rows($vysledek_p)):
				$zaz_p = mysql_fetch_array($vysledek_p);
					extract($zaz_p);
		...
		$vysledek_p = mysql_query("SELECT nazev AS nazev_firmy FROM _pozadavky_zadavatele LEFT JOIN admin ON(_pozadavky_zadavatele.ID_zadavatele=admin.ID) LEFT JOIN firmy ON(admin.ID_firmy=firmy.ID) WHERE _pozadavky_zadavatele.ID_pozadavku='$ID_pozadavku'");
			if(mysql_num_rows($vysledek_p)):
				$zaz_p = mysql_fetch_array($vysledek_p);
					extract($zaz_p);
			endif;
		...
		$vysledek_p = mysql_query("SELECT projekty.zkratka AS nazev_projektu FROM projekty WHERE projekty.ID='$_POST[ID_projektu]'");
			if(mysql_num_rows($vysledek_p)):
				$zaz_p = mysql_fetch_array($vysledek_p);
					extract($zaz_p);
			endif;
		...
		switch($_POST['priorita']):
			case 1: $priorita_cz=gtext('n�zk�',4,'_cz'); $priorita_en=gtext('n�zk�',4,'_en'); break;
			case 2: $priorita_cz=gtext('st�edn�',5,'_cz'); $priorita_en=gtext('st�edn�',5,'_en'); break;
			case 3: $priorita_cz=gtext('vysok�',6,'_cz'); $priorita_en=gtext('vysok�',6,'_en'); break;
		endswitch;
		
		//--zjistit textove hodnoceni
		$vysledek_p = mysql_query("SELECT nazev_cz AS hodnoceni_cz, nazev_en AS hodnoceni_en FROM nastaveni_hodnoceni_pozadavku  WHERE ID='$_POST[ID_hodnoceni]'");
			if(mysql_num_rows($vysledek_p)):
				$zaz_p = mysql_fetch_array($vysledek_p);
					extract($zaz_p);
			endif;
		@mysql_free_result($vysledek_p);
			...
			switch($druh_zpravy):
				case "pozadavek_vlozen":
						$predmet_cz="".gtext('Nov� pozadavek',56,'_cz')." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('zadatel',50,'_cz')." $nazev_firmy)";
						$zprava_cz="".gtext('u�ivatel',57,'_cz')." <i>$autor</i> ($nazev_firmy) <u>".gtext('vlo�il nov� po�adavek',58,'_cz')."</u>.";
						
						$predmet_en="".gtext('Nov� pozadavek',56,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('zadatel',50,'_en')." $nazev_firmy)";
						$zprava_en="".gtext('u�ivatel',57,'_en')." <i>$autor</i> ($nazev_firmy) <u>".gtext('vlo�il nov� po�adavek',58,'_en')."</u>.";
				break;
				...
				case "pridelen_resitel":
						...
						$zprava_cz="<u>".gtext('byl(a) jste ur�en(a) jako �e�itel po�adavku',60,'_cz')."</u> <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i>.";
						
						$predmet_en="".gtext('Prideleni resitele',59,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('zadatel',50,'_en')." $nazev_firmy)";
						$zprava_en="<u>".gtext('byl(a) jste ur�en(a) jako �e�itel po�adavku',60,'_en')."</u> <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i>.";
				...
				case "zmena_textu_spravce":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('byl okomentov�n spr�vcem',140,'_cz')."</u>.";
						
						$predmet_en="".gtext('Vyreseni pozadavku',61,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('zadatel',50,'_en')." $nazev_firmy)";
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('byl vy�e�en',63,'_en')."</u>.";
				...
				case "pozadavek_vyresen":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('byl vy�e�en',63,'_cz')."</u>.";
				...
				case "pozadavek_vyresen_v_nove_verzi":
						$predmet_cz="".gtext('Vyreseni pozadavku v nov� verzi',129,'_cz')." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('zadatel',50,'_cz')." $nazev_firmy)";
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('bude vy�e�en v nov� verzi',130,'_cz')."</u>.";
						
						$predmet_en="".gtext('Vyreseni pozadavku v nov� verzi',129,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('zadatel',50,'_en')." $nazev_firmy)";
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('bude vy�e�en v nov� verzi',130,'_en')."</u>.";
				...
				case "pozadavek_vyresen_schvalit":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('je vy�e�en s prosbou o schv�len� z�kazn�kem',132,'_cz')."</u>.";
						...
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('je vy�e�en s prosbou o schv�len� z�kazn�kem',132,'_en')."</u>.";
				...
				case "pozadavek_vyresen_akceptovan":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('je vy�e�en a odsouhlasen z�kazn�kem',134,'_cz')."</u>.";
						...
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('je vy�e�en a odsouhlasen z�kazn�kem',134,'_en')."</u>.";
				...
				case "pozadavek_vyresen_bude_nove_verzi":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('je vy�e�en a objev� se v nov� verzi',136,'_cz')."</u>.";
						...
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('je vy�e�en a objev� se v nov� verzi',136,'_en')."</u>.";
				...
				case "pozadavek_vracen_uzivateli":
						...
						$zprava_cz="".gtext('�e�itel po�adavku',65,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu)</i> <u>".gtext('��d� o dopln�n� informac�',82,'_cz')."</u>.";
						
						$predmet_en="".gtext('Zadost o doplneni pozadavku',64,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu)";
						$zprava_en="".gtext('�e�itel po�adavku',65,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu)</i> <u>".gtext('��d� o dopln�n� informac�',82,'_en')."</u>.";
				...
				case "pozadavek_vracen_spravci":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('byl vr�cen spr�vci',67,'_cz')."</u>.";
						
						$predmet_en="".gtext('Vraceni pozadavku spravci',66,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('zadatel',50,'_en')." $nazev_firmy)";
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('byl vr�cen spr�vci',67,'_en')."</u>.";
				...
				case "pozadavek_vracen_resiteli":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('byl vr�cen �e�iteli',69,'_cz')."</u>.";
						
						$predmet_en="".gtext('Vraceni pozadavku resiteli',68,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('zadatel',50,'_en')." $nazev_firmy)";
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('byl vr�cen �e�iteli',69,'_en')."</u>.";
				...
				case "pozadavek_doplnen_uzivatelem":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('byl dopln�n u�ivatelem',71,'_cz')."</u>.";
						...
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('byl dopln�n u�ivatelem',71,'_en')."</u>.";
				...
				case "pozadavek_upravena_reakce":
						...
						$zprava_cz="".gtext('po�adavek',62,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('byl upraven v reakci �e�itele',101,'_cz')."</u>.";

						$predmet_en="".gtext('Doplneni reakce resitele',100,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('zadatel',50,'_en')." $nazev_firmy)";
						$zprava_en="".gtext('po�adavek',62,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('byl upraven v reakci �e�itele',101,'_en')."</u>.";
				...
				case "diskuse_pozadavku":
						...
						$zprava_cz="".gtext('k po�adavku',73,'_cz')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_cz')." $nazev_projektu, ".gtext('�adatel',50,'_cz')." $nazev_firmy)</i> <u>".gtext('byl v diskusi p�id�n koment��',74,'_cz')."</u>.";
						
						$predmet_en="".gtext('Diskuse k pozadavku',72,'_en')." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('zadatel',50,'_en')." $nazev_firmy)";
						$zprava_en="".gtext('k po�adavku',73,'_en')." <i>".$_POST['nazev']." (".gtext('Projekt',1,'_en')." $nazev_projektu, ".gtext('�adatel',50,'_en')." $nazev_firmy)</i> <u>".gtext('byl v diskusi p�id�n koment��',74,'_en')."</u>.";
			...
			//--kompletovani textu zpravy
				...
				$zprava_cz.=" ".gtext('Cel� po�adavek m��ete zobrazit zde',75,'_cz').": <a href=\"http://".$_SERVER['SERVER_NAME']."".$_adresa_skriptu."?ID=$ID_pozadavku\" target=\"_blank\">HelpDesk</a>
				...
				<b>".gtext('N�zev po�adavku',9,'_cz').":</b> ".$_POST['nazev']."<br><br>
				<b>".gtext('Po�adavek',10,'_cz').":</b> ".nl2br($_POST['pozadavek'])."<br><br>";
				
				$zprava_en.=" ".gtext('Cel� po�adavek m��ete zobrazit zde',75,'_en').": <a href=\"http://".$_SERVER['SERVER_NAME']."".$_adresa_skriptu."?ID=$ID_pozadavku\" target=\"_blank\">HelpDesk</a>
				<span class=\"text-grey2\"><br><br><b>ID:</b> $ID_pozadavku<br>
				<b>".gtext('Projekt',1,'_en').":</b> ".$nazev_projektu."<br>
				<b>".gtext('Priorita',3,'_en').":</b> ".$priorita_en."<br><br></span>
				<b>".gtext('N�zev po�adavku',9,'_en').":</b> ".$_POST['nazev']."<br><br>
				<b>".gtext('Po�adavek',10,'_en').":</b> ".nl2br($_POST['pozadavek'])."<br><br>";
				
				if($zprava_komentar):
					$zprava_cz.="<b>".gtext('Koment�� spr�vce',12,'_cz').":</b> ".nl2br($_POST['komentar'])."<br><br>";
					$zprava_en.="<b>".gtext('Koment�� spr�vce',12,'_en').":</b> ".nl2br($_POST['komentar'])."<br><br>";
				endif;
				
				if($zprava_reakce):
					$zprava_cz.="<b>".gtext('Reakce �e�itele',14,'_cz').":</b> ".nl2br($_POST['reakce'])."<br><br>";
					$zprava_en.="<b>".gtext('Reakce �e�itele',14,'_en').":</b> ".nl2br($_POST['reakce'])."<br><br>";
				endif;
				
				if($zprava_hodnoceni):
					$zprava_cz.="<b>".gtext('Vy�e�eno',24,'_cz').":</b> $hodnoceni_cz<br>";
					$zprava_en.="<b>".gtext('Vy�e�eno',24,'_en').":</b> $hodnoceni_en<br>";
				endif;
				
				if($zprava_datum_vyreseni):
					$zprava_cz.="<b>".gtext('Datum vy�e�en�',16,'_cz').":</b> ";
					$zprava_en.="<b>".gtext('Datum vy�e�en�',16,'_en').":</b> ";
				...
				if($zprava_diskuse):
					$zprava_cz.="<br><br><b>".gtext('Zadan� p��sp�vek do diskuse',76,'_cz').":</b> ".nl2br($_POST['prispevek'])."<br><br>";
					$zprava_en.="<br><br><b>".gtext('Zadan� p��sp�vek do diskuse',76,'_en').":</b> ".nl2br($_POST['prispevek'])."<br><br>";
				endif;
...
$del_stat=0;
	...
	$SQL=mysql_query("DELETE FROM $tabulka WHERE ID='$ID'");
...
function message($pocet, $hlaska, $odkaz_text, $odkaz){
		...
		if($odkaz_text){
			echo "<br><br><br><a href=\"$odkaz\">$odkaz_text</a>";
		}
		echo "<br>";
...
function presmeruj($url_smerovani,$params=""){
	...
	if($url_smerovani):
		...
		window.location.href = '$url_smerovani"; if($params): echo "?"; endif; echo "$params';
		</script>";
...
function htmlspecialchars_array_encode($zaz){
	foreach($zaz as $klic=>$zaznam):
		$zaz[$klic]=str_replace("&amp;","&",htmlspecialchars($zaznam,ENT_QUOTES));
	endforeach;
...
function insert_tb($tabulka){
	...
	$vysledek = mysql_query("SHOW COLUMNS FROM $tabulka");
	if(mysql_num_rows($vysledek)>0):
		
		$dotaz_1="INSERT INTO $tabulka VALUES (";
		$dotaz_3=")";
		...
		while($zaz = mysql_fetch_array($vysledek)):
			...
			if($dotaz_2):
				$dotaz_2=$dotaz_2.", '".safe($_POST[$sloupec])."'";
			else:
				$dotaz_2=$dotaz_2."'".safe($_POST[$sloupec])."'";
			endif;
...
echo "<SCRIPT language=\"JavaScript\" type=\"text/javascript\">
  ...
  for($i=0;$i<=$pocet_prvku-1;$i++):
	...
	switch($povinne[3][$i]):
		case 1:
			echo "if(!document.".$povinne[0][0].".".$povinne[1][$i].".value)\n
			{����� \n
		...
		case 2:
			echo "if(!document.".$povinne[0][0].".".$povinne[1][$i].".checked)\n
			{����� \n
				document.".$povinne[0][0].".".$povinne[1][$i].".focus();\n
		...
		case 4:
			...
			echo "var hodnota=document.".$povinne[0][0].".".$povinne[1][$i].".value;\n";
...
function strtolower2($vyraz) { 
	$co=iconv('windows-1250','utf-8',$vyraz);
...
function ajax_multibox($ajax_pole1,$ajax_pole2){
	...
	$ajax_ico="<a href=\"javascript:showHint('1-add.php?tab=$_GET[tab]&amp;sl=$_GET[sl]&amp;multi=$_GET[multi]&amp;cil_form=$form_name&amp;cil_id=$nazev_form&amp;cil_text=$cil_text&amp;zpusob=$_GET[zpusob]&amp;ajax_where=$ajax_where&amp;ajax_join=$ajax_join&amp;group_by=$group_by&amp;nadrazeny_form=$nadrazeny_form&amp;podrazeny_form=$podrazeny_form&amp;form='";
		...
		$ajax_ico.="+'&amp;id_value='+document.$form_name.$nazev_form.value,'ajax_form', '1'); minimize('ajax_div')\"><img src=\"img/ico/multi.png\" border=\"0\" width=\"17\" height=\"20\" alt=\"p�i�adit prvky\" style=\"position:relative; top:3px;\" onclick=\"getMouseYObrazovka(event,'ajax_div',150); ztmaveni('meziprostor'); show_it('meziprostor');\"></a>";
				...
				function next_fce_post(){
					...
					foreach($pole_add_cilu as $dilci_pole):
						echo "if(cil_add==\"$dilci_pole[0]\"){";
							...
							echo "ajax_hodnoty('$dilci_pole[1]', cil_add, '".$dilci_pole[1]."'+cil_add, '$dilci_pole[2]', '$dilci_pole[3]', 1, '');
...
function insert_multibox($idcko, $seznam_id, $tabulka_vztahu, $kam_idcko, $typ){
	...
	mysql_query("DELETE FROM $tabulka_vztahu WHERE $sloupce_idcka[0]='$idcko' AND typ='$typ'");
	...
	if($seznam_id):
		...
		foreach($_pole_multi as $multi_id):
			...
			if($multi_id>0):
				...
				mysql_query("INSERT INTO $tabulka_vztahu ($kam_idcko, typ) VALUES
					('$idcko',
					'$multi_id',
					'$typ')");
			endif;	
	...
	if($seznam_id):
		...
		foreach($_pole_multi as $multi_id):
			...
			if($multi_id>0):
				...
				mysql_query("INSERT IGNORE INTO $tabulka_vztahu ($kam_idcko, prava, prava_rozsirujici) VALUES
					('$idcko',
					'$multi_id',
					'$typ',
					'$typ2')");
...
//pou�ije se prim�rn� sirka, vyska je omezuj�c� nastaven�, obr�zek se zmen�� na ���ku a zkontroluje se, zda nen� vy��� ne� zadan� v��ka,
//jestli ano, tak ho je�t� zmen�� na uvedenou v��ku

//nen�-li uvedena ani sirka ani vyska, tak tuto velikost ignoruj
//obrazek je nazev inputu
//primarni je sirka_a - nejv�t�� velikost
//nazev_bd je n�zev, jak se m� obr�zek jmenovat v�etn� adres��e, kde je ulo�en, nap�. "reference/ref"
...
if (is_uploaded_file($_FILES[$obrazek]['tmp_name'])):
   ...
   if(($typ_uploadu==2)||($typ_uploadu==1)){
	...
	list($sirka_img,$vyska_img)=GetImageSize($nazev_docasny_obrazek);	//zji�t�n� ���ky a v��ky origin�ln�ho obr�zku
	...
	if($sirka_a!=0):
		...
		if($sirka_img>$sirka_a):	//1920,800
		   ...
		   //kontrola, zda v��ka nen� v�t�� ne� je zad�n� ve fci
		   //jestli ano, tak p�epo��tej na nov� hodnoty
	...
	if($sirka_a>0):
		...
		$img_cil_a=UnsharpMask($img_cil_a, $amount, $radius, $threshold);  //fce pro ost�en�		
	...
	if($sirka_b>0):
		...
		$img_cil_b=UnsharpMask($img_cil_b, $amount, $radius, $threshold);	//fce pro ost�en�		
	...
	if($sirka_c>0):
		...
		$img_cil_c=UnsharpMask($img_cil_c, $amount, $radius, $threshold);	//fce pro ost�en�		
	...
	echo "<script language=\"JavaScript\" type=\"text/javascript\">
	alert(\"Obr�zek nen� ve form�tu JPG nebo GIF ! Fotografie lze vkl�dat pouze typu JPG nebo GIF.\");
	</SCRIPT>";
...
//-----smazat obr�zek - pro jeden obrazek s pripadnym charakteristickym doplnenim nazvu
...
$pole=array(".jpg",".gif");
 ...
 foreach($pole as $pripona):
	...
	if(File_Exists ($obrazek1)):
		echo "&nbsp;&nbsp;&nbsp;<a href=\"javascript: void window.open('detail_img.php?obrazek_det=$obrazek1','detail','height=50, width=50, scrollbars=no')\" title=\"\">".$ico_lupa2."</a>&nbsp;&nbsp;&nbsp;&nbsp;";
		echo "<a href=\"$_SERVER[PHP_SELF]?del_img=ok&amp;ID=$ID&amp;imd=$imd\" onclick=\"return potvrd('Opravdu si p�ejete smazat obr�zek?\\nNeulo�en� data budou ztracena.')\">".$ico_smazat2."</a>";
	endif;
...
function uloz_soubor($form_pocet,$typ,$akce,$idcko,$dnes){
	...
	if($akce==2):
		...
		$vysledek_fil = mysql_query("SELECT nazev_adresare,nazev_souboru AS stary_soubor,poradove_cislo FROM _soubory WHERE ID_dokumentu='$idcko' AND typ='$typ' ORDER BY poradove_cislo");
		...
		for($k=1;$k<=$form_pocet;$k++):
			...
			if($soubor_name):
				...
				mysql_query("DELETE FROM _soubory WHERE ID_dokumentu='$idcko' AND poradove_cislo='$k' AND typ='$typ'");
...
if (is_uploaded_file($_FILES[$file]['tmp_name'])):
	...
	$soubor_name=StrTr($_FILES[$file]['name'], "������������������������������؊����ݎ:/;�,� ", "aacdeeeinoorstuuuyzAACDEEEINOORSTUUUYZ-------");
	...
	if($pripona!="php"&&$pripona!="asp"&&$pripona!="php3"&&$pripona!="php4"):
	  ...
	  if($stary_soubor):
		...
		echo "<script language=\"JavaScript\" type=\"text/javascript\">
		alert(\"Soubory .$pripona nelze nahr�t!\");
		</SCRIPT>";
...
function file_fce_del($ID,$soubor,$poradi,$soubor_typ){ //imd je rozliseni obrazku, pokud jich je k obsahu vice | barva 1 = svetlejsi radek, barva 2 tmavsi
	...
	if (File_Exists ($soubor)):
		echo "<a href=\"$_SERVER[PHP_SELF]?del_file=ok&amp;ID=$ID&amp;poradi=$poradi&amp;soubor_typ=$soubor_typ\" title=\"smazat\" onclick=\"return potvrd('Opravdu si p�ejete smazat soubor?')\"><img src=\"img/ico/smazat.png\" border=\"0\" width=\"17\" height=\"20\" alt=\"smazat\" style=\"position: relative; top: 4px\"></a>";
	endif;
...
global $FILE_PATH;
	...
	if (File_Exists ($soubor_del)):
		...
		mysql_query("DELETE FROM _soubory WHERE ID_dokumentu='$ID' AND poradove_cislo='$poradi' AND typ='$soubor_typ'");
		
		$vysledek_fil = mysql_query("SELECT nazev_adresare FROM _soubory WHERE ID_dokumentu='$ID' AND typ='$soubor_typ'");
...
global $FILE_PATH;
	...
	if (File_Exists ($soubor)):
		...
		echo "<a href=\"$soubor\" target=\"_blank\">$nazev_souboru</a>";
		echo "</div>";
	endif;
...
function file_fce_ico($soubor,$top,$alt){ //imd je rozliseni obrazku, pokud jich je k obsahu vice | barva 1 = svetlejsi radek, barva 2 tmavsi
	...
	if (File_Exists ($soubor)):
		...
		echo "<a href=\"$soubor\" target=\"_blank\" title=\"$alt\"><img src=\"img/ico/$ico_file.png\" border=\"0\" width=\"17\" height=\"18\" alt=\"$alt\" style=\"position: relative; top: ".$top."px\"></a>";
	endif;
