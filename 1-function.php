...
function cleanInput($value, $link = '')
{
    ...
    {
    	return clean_basic(strip_tags(trim($value)), $link);
    }
}

//Whitelist post variables
foreach ($_POST as $key => $value)
{
    $_POST[$key] = cleanInput($value, $link);
}

//Whitelist get variables
...
function strankovani($dotaz, $_na_stranku, $pocitej_sloupec="*"){
	...
	$zaznamu=mysql_result(mysql_query("SELECT COUNT(".mysql_real_escape_string($pocitej_sloupec).") FROM ".mysql_real_escape_string($dotaz).""), 0);
...
function vypsat_tabulku($_poradi_default,$cast){
	...
	$vysledek_p = mysql_query("SELECT ID_sloupce FROM _uzivatele_sloupce WHERE ID_uzivatele='".clean_high($_SESSION[usr_ID])."' ORDER BY poradi");
...
function get_id_adresatu($ID_projektu,$prava,$prava_rozsirujici){
	...
	$vysledek_p = mysql_query("SELECT ID_uzivatele AS id_adresata FROM admin_prava_projekty WHERE ID_projektu='".clean_high($ID_projektu)."' AND prava='".mysql_real_escape_string($prava)."' AND prava_rozsirujici='".mysql_real_escape_string($prava_rozsirujici)."'");
		if(mysql_num_rows($vysledek_p)):
			while($zaz_p = mysql_fetch_array($vysledek_p)):
				extract($zaz_p);
...
function get_id_adresatu_zadavatelu($ID_pozadavku){
	...
	$vysledek_p = mysql_query("SELECT ID_zadavatele AS id_adresata FROM _pozadavky_zadavatele WHERE ID_pozadavku='".clean_high($ID_pozadavku)."'");
		if(mysql_num_rows($vysledek_p)):
			while($zaz_p = mysql_fetch_array($vysledek_p)):
				extract($zaz_p);
				$_pole_id_adresatu[]=$id_adresata;
...
function zasli_email_adresatum($_pole_id_adresatu,$druh_zpravy,$autor_udalosti_id,$ID_pozadavku){
	...
	if(count($_pole_id_adresatu)>0):
		...
		$vysledek_p = mysql_query("SELECT CONCAT(jmeno,' ',prijmeni) AS autor, admin.email AS email_od FROM admin WHERE admin.ID='".clean_high($autor_udalosti_id)."'");
			if(mysql_num_rows($vysledek_p)):
		...
		$vysledek_p = mysql_query("SELECT ID_projektu, nazev AS nazev_poz, pozadavek AS poz, reakce AS reakce_poz FROM pozadavky WHERE ID='".clean_high($ID_pozadavku)."'");
			if(mysql_num_rows($vysledek_p)):
				$zaz_p = mysql_fetch_array($vysledek_p);
					extract($zaz_p);
		...
		$vysledek_p = mysql_query("SELECT nazev AS nazev_firmy FROM _pozadavky_zadavatele LEFT JOIN admin ON(_pozadavky_zadavatele.ID_zadavatele=admin.ID) LEFT JOIN firmy ON(admin.ID_firmy=firmy.ID) WHERE _pozadavky_zadavatele.ID_pozadavku='".clean_high($ID_pozadavku)."'");
			if(mysql_num_rows($vysledek_p)):
				$zaz_p = mysql_fetch_array($vysledek_p);
					extract($zaz_p);
			endif;
		...
		$vysledek_p = mysql_query("SELECT projekty.zkratka AS nazev_projektu FROM projekty WHERE projekty.ID='".clean_high($_POST['ID_projektu'])."'");
			if(mysql_num_rows($vysledek_p)):
				$zaz_p = mysql_fetch_array($vysledek_p);
					extract($zaz_p);
			endif;
		...
		$vysledek_p = mysql_query("SELECT nazev_cz AS hodnoceni_cz, nazev_en AS hodnoceni_en FROM nastaveni_hodnoceni_pozadavku  WHERE ID='".clean_high($_POST['ID_hodnoceni'])."'");
			if(mysql_num_rows($vysledek_p)):
				$zaz_p = mysql_fetch_array($vysledek_p);
					extract($zaz_p);
			endif;
...
$del_stat=0;
	...
	$SQL=mysql_query("DELETE FROM ".clean_high($tabulka)." WHERE ID='".clean_high($ID)."'");
...
function insert_multibox($idcko, $seznam_id, $tabulka_vztahu, $kam_idcko, $typ){
	...
	mysql_query("DELETE FROM ".clean_high($tabulka_vztahu)." WHERE $sloupce_idcka[0]='".mysql_real_escape_string($idcko)."' AND typ='".mysql_real_escape_string($typ)."'");
...
function uloz_soubor($form_pocet,$typ,$akce,$idcko,$dnes){
			...
			for($k=1;$k<=$form_pocet;$k++):
				...
				if($soubor_name):
					mysql_query("INSERT INTO _soubory VALUES
						('".clean_basic($idcko)."',
						'".clean_basic($nazev_adresare)."',
						'".clean_basic($soubor_name)."',
						'".clean_basic($k)."',
						'".clean_basic($typ)."',
						'".clean_basic($dnes)."')");
				endif;
	...
	if($akce==2):
		...
		$vysledek_fil = mysql_query("SELECT nazev_adresare,nazev_souboru AS stary_soubor,poradove_cislo FROM _soubory WHERE ID_dokumentu='".clean_high($idcko)."' AND typ='".mysql_real_escape_string($typ)."' ORDER BY poradove_cislo");
		...
		for($k=1;$k<=$form_pocet;$k++):
			...
			if($soubor_name):
				...
				mysql_query("DELETE FROM _soubory WHERE ID_dokumentu='".clean_high($idcko)."' AND poradove_cislo='".mysql_real_escape_string($k)."' AND typ='".mysql_real_escape_string($typ)."'");
...
global $FILE_PATH;
	...
	if (File_Exists ($soubor_del)):
		...
		$vysledek_fil = mysql_query("SELECT nazev_adresare FROM _soubory WHERE ID_dokumentu='".clean_high($ID)."' AND typ='".mysql_real_escape_string($soubor_typ)."'");
