...
if($_POST['action']=="login"):
	...
	if($_POST['login']&&$_POST['heslo']):
		...
		//Check password for new bcrypt hash -AF
		$stmt = $mysqli->prepare("SELECT ID, ID_centra, ID_registru, jazyk_vychozi, ID_role_admin FROM admin WHERE login=? AND heslo=? AND (platnost_od<='".$dnes."' AND (platnost_do='0' OR platnost_do+86380>='".$dnes."')) LIMIT 1");
		$stmt->bind_param("ss", $uziv_jmeno_prepared_stmt, $uziv_heslo_bcrypt);
		$stmt->execute();
		...
		//Check password for old SHA1 hash if bcrypt does not match -AF
		//if($ID=0):
		$stmt = $mysqli->prepare("SELECT ID, ID_centra, ID_registru, jazyk_vychozi, ID_role_admin FROM admin WHERE login=? AND heslo=? AND (platnost_od<='".$dnes."' AND (platnost_do='0' OR platnost_do+86380>='".$dnes."')) LIMIT 1");
		$stmt->bind_param("ss", $uziv_jmeno_prepared_stmt, $uziv_heslo);
		$stmt->execute();
...
<?
	if($_GET['st']):
		...
		if($_GET['st']=="bad"):
			echo gtext('Zadané údaje jsou chybné',105).".!".$ID=0;
		endif;
