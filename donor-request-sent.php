...
//-----vypis dat
$vysledek = mysql_query("SELECT typing_request.ID, typing_request.patient_name, typing_request.PatientNum, typing_request.date_birth, typing_request.diagnosis, typing_request.RegID,
	typing_request_donor.DonorID, typing_request_donor.DonorNumber, typing_request_donor.ID_stavu, typing_request_donor.poznamka, typing_request_donor.duvod 
	FROM typing_request_donor LEFT JOIN typing_request ON(typing_request_donor.ID_typing=typing_request.ID) 
	WHERE ID_stavu IN(0,2) $where
		...
		echo "<table cellspacing=\"0\" width=\"100%\">
			<thead id=\"tb-head\">
			  <tr>
				...
				<td width=\"17%\">".order('Donor ID', 'DonorID')."</td>
			...
			while($zaz = mysql_fetch_array($vysledek)){
				...
				echo "<tr ".$styl.">";
					...
					if($ID_stavu==0 || $ID_stavu==2):
						echo "<a href=\"$_SERVER[PHP_SELF]?action=delete&amp;ID=$ID&amp;DonorNum=$DonorNumber\" title=\"DELETE\" onclick=\"return potvrd('Do you really remove this item?')\">".$ico_smazat."</a>";
					endif;
